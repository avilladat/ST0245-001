...
public class DigraphAL extends Digraph {
    ...
    public void addArc(int source, int destination, int weight) throws Exception {
        ...
        list.get(destination).addLast(new Pair<Integer, Integer>(source, weight));
    }
     ...
     */
    public Queue<Integer> getSuccessors(int vertex) throws Exception {
        if (vertex >= size)
    ...
    public void printCodeForGraphViz() {
        for (int i = 0; i < size; i++) {
            for (int j = 0; j < list.get(i).size(); j++) {
                System.out.println("\"" + i + "\" -> \"" + list.get(i).get(j).first + "\"[ label = \""
